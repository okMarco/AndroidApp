package com.hochan.tumlodr.util.statusbar;

import android.annotation.SuppressLint;
import android.os.Environment;

import java.io.File;
import java.io.FileInputStream;
import java.io.IOException;
import java.lang.reflect.Method;
import java.util.Properties;

/**
 * .
 * Created by czd on 2018/1/27.
 */

public class PhoneSystemCompat {

	public static final String SYS_EMUI = "sys_emui";
	public static final String SYS_MIUI = "sys_miui";
	public static final String SYS_FLYME = "sys_flyme";
	public static final String SYS_OPPO = "sys_oppo";
	public static final String SYS_SMARTISAN = "sys_smartisan";
	public static final String SYS_UNKNOWN = "sys_unknown";

	// 小米
	private static final String KEY_MIUI_VERSION_CODE = "ro.miui.ui.version.code";
	private static final String KEY_MIUI_VERSION_NAME = "ro.miui.ui.version.name";
	private static final String KEY_MIUI_INTERNAL_STORAGE = "ro.miui.internal.storage";

	// 华为
	private static final String KEY_EMUI_API_LEVEL = "ro.build.hw_emui_api_level";
	private static final String KEY_EMUI_VERSION = "ro.build.version.emui";
	private static final String KEY_EMUI_CONFIG_HW_SYS_VERSION = "ro.confg.hw_systemversion";

	//OPPO
	private static final String KEY_OPPOROM_VERSION_CODE = "ro.build.version.opporom";

	// VIVO
	private static final String KEY_VIVO_VERSION_CODE = "ro.vivo.os.version";

	// 锤子
	private static final String KEY_SMARTISAN_VERSION_CODE = "ro.smartisan.version";

	public static String getSystem() {
		String SYS = "";
		try {
			Properties prop = new Properties();
			prop.load(new FileInputStream(new File(Environment.getRootDirectory(), "build.prop")));
			if (prop.getProperty(KEY_MIUI_VERSION_CODE, null) != null
					|| prop.getProperty(KEY_MIUI_VERSION_NAME, null) != null
					|| prop.getProperty(KEY_MIUI_INTERNAL_STORAGE, null) != null) {
				SYS = SYS_MIUI;// 小米
			} else if (prop.getProperty(KEY_EMUI_API_LEVEL, null) != null
					|| prop.getProperty(KEY_EMUI_VERSION, null) != null
					|| prop.getProperty(KEY_EMUI_CONFIG_HW_SYS_VERSION, null) != null) {
				SYS = SYS_EMUI;// 华为
			} else if (getMeizuFlymeOSFlag().toLowerCase().contains("flyme")) {
				SYS = SYS_FLYME;// 魅族
			} else if (prop.getProperty(KEY_OPPOROM_VERSION_CODE, null) != null) {
				SYS = SYS_OPPO;
			} else if (prop.getProperty(KEY_SMARTISAN_VERSION_CODE, null) != null) {
				SYS = SYS_SMARTISAN;
			} else {
				return SYS_UNKNOWN;
			}
		} catch (IOException e) {
			e.printStackTrace();
			return SYS;
		}
		return SYS;
	}

	public static String getMeizuFlymeOSFlag() {
		return getSystemProperty("ro.build.display.id", "");
	}

	private static String getSystemProperty(String key, String defaultValue) {
		try {
			@SuppressLint("PrivateApi")
			Class<?> clz = Class.forName("android.os.SystemProperties");
			Method get = clz.getMethod("get", String.class, String.class);
			return (String) get.invoke(clz, key, defaultValue);
		} catch (Exception e) {
			// nothing
		}
		return defaultValue;
	}

	public static boolean isMIUI() {
		return getSystem().equals(SYS_MIUI);
	}

	public static boolean isEMUI() {
		return getSystem().equals(SYS_EMUI);
	}

	public static boolean isMEIZU() {
		return getSystem().equals(SYS_FLYME);
	}

	public static boolean isOPPO() {
		return getSystem().equals(SYS_OPPO);
	}

	public static boolean isSMARTISAN() {
		return getSystem().equals(SYS_SMARTISAN);
	}
}
